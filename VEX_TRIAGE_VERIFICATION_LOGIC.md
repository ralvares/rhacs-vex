...
* **Logic — three tiers, most specific first:**
    1. **OCI purl / SHA-exact match:** The engine builds OCI identity candidates from **both** the image reference and the `name` label, then matches them against the purls in the VEX product tree — exact `repository_url` first, purl package-name equality as fallback (bridges Brew label namespaces like `managed-open-data-hub/` to registry namespaces like `rhoai/` without hardcoded rewrites). Entries whose PID carries the scanned image's **exact SHA256 digest** are an assessment of *this build* and **override all generic entries** — e.g. a generic `known_affected` plus an our-digest `known_not_affected` means our build already contains the fix → **FALSE POSITIVE**. This digest override runs engine-wide **before the RPM/non-RPM branch**: a VEX statement about our exact build settles the verdict for *every* component in the image, RPMs included (affected wins over cleared when both digest entries exist).
...
* **Vendor severity, most specific first:** Severity is resolved from VEX per-product threat ratings through a priority chain (OCI purl match → image-level PID → component-name PID → in-scope affected/fixed PID → document aggregate → generic threat → CVSS → scanner severity), so the reported rating reflects Red Hat's assessment for *this* product, not a generic CVSS score. Within the purl step, our-digest ratings beat generic ones; ratings from *other* builds are used only when they all agree. Findings where the scanner and VEX disagree are flagged.
* **Remediation state:** Every finding also carries Red Hat's CVE-page **State** (`VEX_STATE`): Fixed, Not affected, Fix available, Under investigation, Will not fix, Fix deferred, or Affected — derived from the verdict plus `no_fix_planned` / `none_available` remediation entries, using Red Hat's own `details` wording.
