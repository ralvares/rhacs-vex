...
def rhacs_scan_image(session, image_ref: str) -> Optional[dict]:
    ...
    Calls POST /v1/images/scan; the CachedSession handles caching automatically.
    Returns the full image dict (including scan data) or None on failure.
    """
    try:
        resp = session.post(f"{session.base_url}/v1/images/scan",
                            json={"imageName": image_ref, "force": False}, timeout=120)
        resp.raise_for_status()
        data = resp.json()
...
def _build_sbom_src_map(sbom: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            for rel in parser.get_relationships()
...
def main():
    ...
    if use_api:
        ...
        try:
            ...
            _url   = f"{session.base_url}/v1/images/{image_id}"
            if not _force and session.cache.contains(url=_url):
                _console.print("  [dim]Using cached scan[/dim]")
